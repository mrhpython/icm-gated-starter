...
const DRAFT_SCHEMA = {
  type: 'object', required: ['file', 'sections', 'word_count', 'marker_removed'], additionalProperties: false,
  properties: {
    file: { type: 'string' },
...
Return file, sections (headings in order), word_count, marker_removed.`,
  { label: 'draft', phase: 'Draft', schema: DRAFT_SCHEMA })
if (!draft) throw new Error('draft stage returned nothing')
log(`Draft: ${draft.sections.length} sections, ${draft.word_count} words`)
