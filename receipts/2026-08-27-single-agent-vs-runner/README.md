...
## Runs 2 and 3, the same day, pre-registered

We then ran it the way the last line above asks for. Run 2: five tasks, four arms, no user-level instruction file in scope. The drafting stage repaired the planted defects in both lines before any verifier saw them, so that run could not answer the verifier question; it did answer one thing, five times out of five: one prompt does not cross the stage boundary. Run 3: two defects injected into each finished draft, three verify-only arms (the writer's own session, a fresh session, the runner's verify agent). Every arm flagged both defects on all five tasks. Fifteen FAIL verdicts, no false pass, no edits during verify. The check itself caught them: an injected sentence contradicted the rest of the draft, and the contradiction lens fired for whoever ran it.

So the single observation above stays a single observation. What the runs establish instead: a separate reader is the shape we ship because a writer cannot be the judge of its own judgment, and on defects the check can see, the writer's own session did as well as a separate reader. The runner's receipted advantage is that it crosses the stage boundary without a person. No false pass in thirty-five verify runs. The next design, if it runs, plants a defect the check cannot see, so that only the reading against the brief can catch it.
