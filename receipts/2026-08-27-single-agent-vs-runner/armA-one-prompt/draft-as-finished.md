...
at api.soulfield.one that needs no key and allows a few calls per day per address, enough to try it
...
We could not show that staging alone beats a bare draft. The folders make the work easier to follow.
Whether they make it more correct, we could not show, and we say so.
