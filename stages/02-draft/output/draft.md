...
- Why a verify stage (the independent checker: a separate model, fixed checks, inspectable
  findings, UNKNOWN when a check cannot complete; runs locally for free or against any endpoint;
  the grep for claims you have ruled out; the mutant that must fail every run; nothing here waits
  for a person)
- What we do not claim (no catch rate for your data; we could not show that staging alone beats a
  bare draft and we publish that; a PASS is not clearance; a person owns publishing)
- Get it (clone, read SETUP.md, run one week of real work through it; no email address)
